...
class Detector():
    ...
    def __call__(self, img, threshold=0.5):
        """
        metodo che effettua la object detection
        Args:
            img: (numpy array) frame acquisito dalla camera di pepper
            thresold: (float) valore di confidenza della rete, 0.5 di default
        Returns:
            detections: (list) lista con le detection della rete che superano il threshold
        """
        img = img[:,:,::-1]
